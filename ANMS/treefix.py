...
# 		global list_tree 
		if(type(dic)==dict):
			for item in dic.items():        
				...
				self.list_tree.append(seperator + item[0])      
				self.showTree(item[1], seperator + " | ")
		else:
			print(seperator + " -> (", dic +")")
			self.list_tree.append(seperator + " -> (" + dic +")")

	def vis_Tree(self, dic):
# 		global list_tree 
		if(type(dic)==dict):
			for i,item in enumerate (dic.items()):        
# 				print(i,item[0],item)
				if len(self.list_tree) == 3 :
					if self.list_tree[0] in attrs and self.list_tree[1] not in attrs and self.list_tree[2] in attrs:
						a = str(self.list_tree[0]) + '->' + str(self.list_tree[2])
						b = self.list_tree[2]
                    
						self.list_tree = [b]
						print(a)
				self.list_tree.append(item[0])      
				self.vis_Tree(item[1])
			print('a')
                
		else:
# 			print(" -> (", dic +")")
# 			self.list_tree.append( " -> (" + dic +")")
			pass


	def convertTree(self,dic, parent):
		output_tree = list()
		if (type(dic) == dict):
			for item in dic.items():
				if (parent == None):
					output_tree.append({
						"name": item[0],
						"children": self.convertTree(item[1], item[0])
					})
				else:
					output_tree.append({
						"name": item[0],
...
# tree2 = DecisionTree()
# tr_data, clss, attrs = tree.read_data('DataDiskrit.csv')

# tree2 = tree.create_tree(tr_data, clss, attrs)
